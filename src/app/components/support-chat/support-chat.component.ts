...
export class SupportChatComponent implements OnInit, OnDestroy {
  ...
  getOnlineRooms() {
    this.socket.on("online rooms", (roomsData: string[]) => {
      for (let i = 0; i < roomsData.length; i++) {
        this.rooms[i] = {roomName:roomsData[i],status:true}        
      }
  ...
  isOnline() {
    this.socket.on("offline person", (userGuid: string) => {
      let indexOfOfflinePerson = this.rooms.findIndex(e=>e.roomName == userGuid)
      console.log(indexOfOfflinePerson)
    })
  }
